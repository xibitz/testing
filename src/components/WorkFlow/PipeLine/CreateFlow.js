  ...
  const onConnect = (params) => {
    ...
    setElements((els) =>
      addEdge(
        {
          ...params,
          id: "e2b-" + params.source + "-" + params.target,
          animated: true,
 ...
 */
  ...
  const submit = () => {
    ...
    let value = JSON.stringify(specs, undefined, 3);
  ...
  const onDrop = (event) => {
    ...
    const el = {
      ...
      data: {
        ...
        onChange: onChange,
        color: initBgColor,
        colorService: Colors.colorService[item]
      },
      position: position,
      targetPosition: "left"
    };
      ...
      <ThemeProvider theme={theme}>
        <Col md={{ span: 12, offset: 0 }}>
          <ReactFlowProvider>
            <Row>
              <Col
                className="block-example border-right border-gray"
                style={{ padding: "0px 0px 0px -5px", marginLeft: -7 }}
                md={{ span: 1, offset: 0 }}
              >
                <Typography
                  variant="body1"
                  style={{ padding: "0px 0px 0px -5px", marginLeft: -12 }}
                >
                  ToolBox{" "}
                </Typography>
                <ServiceToolBar />
              ...
              >
                <Box
                  py={0}
                  pr={5}
                  className="block-example border-bottom border-gray"
                >
                  <Box
                    flexShrink={0}
                    display="flex"
                  ...
                  >
                    ...
                    <Box
                      py={1}
                      pr={5}
                      style={{ marginLeft: 5, marginTop: 0, padding: 0 }}
                    >
              ...
              <Col md={{ span: 2, offset: 0 }} style={{ padding: "0px" }}>
                <Box
                  style={{ width: "250px", height: "455px", padding: "2px" }}
                >
                  {highlighted && highlighted === "Collector" && (
                    <CollectorEdit highlighted={highlighted} />
                  )}
                  {highlighted && highlighted === "Curator" && (
                    <CuratorEdit highlighted={highlighted} />
                  )}
                  {highlighted && highlighted === "Transformer" && (
                    <TransformerEdit highlighted={highlighted} />
                  )}
                  {highlighted && highlighted === "Publisher" && (
                    <PublisherEdit highlighted={highlighted} />
                  )}
                  {highlighted && highlighted === "Mapper" && (
                    <TransformerEdit highlighted={highlighted} />
                  )}
                </Box>
              </Col>
