...
function ContainsButModified(arrA, field) {
  for (var i = 0; i < arrA.length; i++) {
    if (
      arrA[i].name === field.name &&
      arrA[i].spec !== field.spec &&
      field.spec &&
      field.spec.includes("(@")
    ) {
      return true;
...
function newButComplexTransform(arrA, field) {
  for (var i = 0; i < arrA.length; i++) {
    if (
      arrA[i].name !== field.name &&
      arrA[i].name !== field.spec &&
      field.spec &&
      field.spec.includes(arrA[i].name)
    ) {
      return true;
    }
  }
...
class ServiceComponent extends Component {
  ...
  handleDelete(name) {
    ...
    outputSchema.fields = this.state.outputSchema.fields.filter(
      (el) => el.name != name
    );
  ...
  generateSpec(inputSchema, outputSchema) {
    ...
    if (sameMembers(inputSchema.fields, outputSchema.fields)) {
      ...
      inputSchema.fields.forEach((element) => {
        if (
          !(
            Contains(outputSchema.fields, element) ||
            ContainsButNameModified(outputSchema.fields, element)
          )
        ) {
      ...
      if (removeds.length > 0) {
        ...
        if (isAnyItemAdded) joltSpec.push(joltModified);
      }
    }
          ...
          <Container mr={5}>
            <Row>
              <Col
                ...
                md={{ span: 1, offset: 0 }}
              >
                <Box py={0} mt={21} borderRight="1px" borderColor="#000"></Box>
              </Col>
              <Col
                md={{ span: 11, offset: 0 }}
